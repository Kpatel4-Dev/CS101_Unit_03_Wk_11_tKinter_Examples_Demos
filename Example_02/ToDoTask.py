...
def clear_taskNumberField() :
    ...
    enterNumberfield.delete(0.0, END)
...
def insertTask():
  ...
  #if you want to print the list upon exiting the program
def on_exit():
    #printing the tasks to the file
    file_path = "ToDoTasks.txt"
    for item in tasks_list:
        ...
        print("[" + str(counter-1) + " tasks to complete in the list]")

    #print(tasks_list) 
    #--> this prints list in list format
    # handling the empty task error
    if len(tasks_list) == 0:
        messagebox.showerror("Tasks", "No tasks to complete! :o)")
        return
...
def delete():
    ...
    number = enterNumberfield.get(1.0, END)
...
if __name__ == "__main__" :
    ...
    Submit = Button(gui, text = "Submit", fg = "white", bg = "Red", command = insertTask)  
    ...
    enterNumberfield = Text(gui, height = 1, width = 2, font = "lucida 13")
    ...
    # when user press the button, the command or 
    # function affiliated to that button is executed .
    Exit = Button(gui, text = "Exit", fg = "white", bg = "Red", command = on_exit)
    ...
    enterNumberfield.grid(row = 5, column = 2)
